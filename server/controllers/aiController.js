...
export const generateArticle = async (req, res) => {
  try {
    ...
    const response = await groq.chat.completions.create({
      ...
      temperature: 0.6,
      max_tokens: Math.min(length, 500), // 🔒 SAFE LIMIT
    });
...
export const generateBlogTitle = async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ success: false, message: "Blog title generation failed" });
  }
};
...
export const resumeReview = async (req, res) => {
  try {
    ...
    const prompt = `Review the following resume and give detailed constructive feedback:\n\n${pdfData.text}`;
